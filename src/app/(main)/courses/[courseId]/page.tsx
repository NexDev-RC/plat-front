...
import { Clock, Users, BookOpen, Globe, Award, CheckCircle, Plus, Pencil } from 'lucide-react'
...
export default async function CourseDetailPage({ params }: Props) {
  ...
  // Leer rol del usuario desde la cookie JWT decodificada (disponible en auth store)
  // El rol lo manejamos client-side en EnrollButton, aquí solo necesitamos el curso
        ...
        <div className="container-page py-10 lg:flex lg:items-start lg:gap-10">
          <div className="flex-1">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-4">
              <Rating value={course.rating} count={course.totalReviews} />
              <span className="flex items-center gap-1.5 text-sm text-gray-300">
                <Users className="h-4 w-4" />{course.totalStudents.toLocaleString()} estudiantes
              </span>
              <span className="flex items-center gap-1.5 text-sm text-gray-300">
            ...
            <p className="mt-3 text-sm text-gray-400">
              Instructor: <span className="font-medium text-primary-400">{course.instructor.name}</span>
            </p>
          </div>
        ...
        <div className="flex-1 space-y-10">
          ...
          {course.tags.length > 0 && (
            <section>
              ...
              <div className="grid gap-2 rounded-xl border border-gray-200 p-5 dark:border-gray-800 sm:grid-cols-2">
                {course.tags.map((tag) => (
          ...
          {course.sections.length > 0 && (
            <section>
              ...
              <div className="mb-3 flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500">
                ...
                <span>·</span>
                <span>{formatDuration(course.totalDuration)} de duración total</span>
              </div>
                  ...
                  .map((section) => (
                    <details key={section.id} className="group">
                      <summary className="flex cursor-pointer items-center justify-between p-4 font-medium text-gray-900 hover:bg-gray-50 dark:text-gray-100 dark:hover:bg-gray-900">
                        ...
                        <span className="text-sm font-normal text-gray-400">{section.lessons.length} lecciones</span>
                      </summary>
                      <ul className="divide-y divide-gray-100 bg-gray-50 dark:divide-gray-800 dark:bg-gray-900/50">
                        {section.lessons
                          .slice()
                          .sort((a, b) => a.order - b.order)
                          .map((lesson) => (
                            <li key={lesson.id} className="flex items-center justify-between px-5 py-3 text-sm">
                              <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                                ...
                                {lesson.isFree && <Badge variant="primary" className="text-[10px]">Gratis</Badge>}
                              </div>
            ...
            <div className="flex items-start gap-4 rounded-xl border border-gray-200 p-5 dark:border-gray-800">
              <div className="h-16 w-16 shrink-0 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                {course.instructor.avatarUrl
                  ...
                  : <div className="flex h-full w-full items-center justify-center text-2xl font-bold text-gray-400">{course.instructor.name[0]}</div>
                }
              </div>
              <div>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="p-5">
        ...
        {/* EnrollButton gestiona student / admin / no autenticado */}
        ...
        <ul className="mt-5 space-y-2 text-sm text-gray-600 dark:text-gray-400">
          <li className="flex items-center gap-2"><Clock    className="h-4 w-4 shrink-0 text-gray-400" />{formatDuration(course.totalDuration)} de contenido</li>
          <li className="flex items-center gap-2"><BookOpen className="h-4 w-4 shrink-0 text-gray-400" />{totalLessons} lecciones</li>
          <li className="flex items-center gap-2"><Award    className="h-4 w-4 shrink-0 text-gray-400" />Certificado al completar</li>
          <li className="flex items-center gap-2"><Globe    className="h-4 w-4 shrink-0 text-gray-400" />Acceso de por vida</li>
        </ul>
      </div>
    </div>
  )
